...
_, out2, _ = c.exec_command('curl -s -o /dev/null -w "%{http_code}" https://api.aidoktor.uz/health/')
time.sleep(4)
print('Health check:', out2.read().decode('utf-8', errors='replace'))
_, out3, _ = c.exec_command('curl -s -I -H "Origin: https://aidoktor.uz" https://api.aidoktor.uz/api/auth/profile/ 2>&1 | head -20')
time.sleep(4)
print('CORS test:')
